...
void ADC14_IRQHandler(void)
{
    ...
    if(status & ADC_INT1)
    {
        ...
        if(resultsBuffer[0]>11000 && (current_state_run==STATE_DESELECT || current_state_run==STATE_LEFT)){
            jevent=JR_PRESSED;
        }

        if(resultsBuffer[0]<5000 && (current_state_run==STATE_DESELECT || current_state_run==STATE_RIGHT)){
              jevent=JL_PRESSED;
        }
        ...
        if(current_state==STATE_ARRIVED){
            if(resultsBuffer[0]>11000 && joystick_pressed){
        ...
        if(resultsBuffer[0]<5000 && joystick_pressed){
